...
% 3-Step BMA when used for motion estimation on a series of video frames as
% it would be within a video compression algorithm.
%% INPUTS AND OUTPUTS
% INPUTS: Anchor frame, Target frame, Block size, Search range,
% OUTPUTS: Motion Vector
%% START OF CODE

function [dmi, dmj, predicted_i] = three_step_GB(anchor_i, target_i, BlockSize, S_Range)
tic;
% -------------------------------------------------------------------------
...
for Step = 1:1:3
    ...
    for i = 1:1:BlockPartitioned(1,1)       % Stepping through each block horizontally
        for j = 1:1:BlockPartitioned(1,2)   % Stepping through each block vertically
            ...
            % Stepping through each candidate in the search range, note
            % step size
            for k = max(1,i1-Ri):Ri:min(m-N+1,i1+Ri)
                for l = max(1,j1-Rj):Rj:min(n-N+1,j1+Rj)
                    if k==0 && l==0 && Step ~= 1 % Does not compare center point for steps 2 and 3
                    else
